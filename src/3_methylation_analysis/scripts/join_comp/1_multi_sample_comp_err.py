...
def process_chromosome(int_chr):
    ...
    check_file=f"{new_folder_path}chr{chr}.filter.txt"
    if three_locations_file != 0 and os.path.exists(check_file)==False:
...
if __name__ == "__main__":
    ...
    filter_reads_list=[2,4,5]
    for filter_reads in filter_reads_list:
        chromosome_range=range(1,25)
        old_folder_path=f"/public/home/xiayini/project/nanopore_ATRX/3_methylation_analysis/results/filter{str(filter_reads)}Reads"
